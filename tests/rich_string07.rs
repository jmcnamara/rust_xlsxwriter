...
fn create_new_xlsx_file(filename: &str) -> Result<(), XlsxError> {
    ...
    worksheet.write_rich_string_only(3, 1, &segments)?;

    let segments = [(&default, "a"), (&bold, "bc"), (&default, "defg")];
    worksheet.write_rich_string_only(4, 2, &segments)?;

    let segments = [(&default, "abc"), (&italic, "de"), (&default, "fg")];
    worksheet.write_rich_string_only(5, 3, &segments)?;

    let segments = [(&default, "a"), (&bold, "bcdef"), (&default, "g")];
    worksheet.write_rich_string_only(6, 4, &segments)?;

    let segments = [(&italic, "abcd"), (&default, "efg")];
    worksheet.write_rich_string_only(7, 5, &segments)?;
